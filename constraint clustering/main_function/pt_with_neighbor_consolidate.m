function pt_with_neighbor_consolidate (nq, nps, trainData, testData, wholeData, datasetName, num, flag1, flag2, exploreMethod)
n1 = length(trainData);
n2 = length(testData);
n = n1+n2;
bcount = 0;
...
kkkk = 25;
cccc = min (kkkk,j);
[neighborhood, selectPairs] = consolidate (neighborhood, cccc, wholeData, selectPairs, trainData);
...
cd (datasetName);
if isequal (exploreMethod, 'normal') 
    fileName = [datasetName, '-pt-with-neighbor-consolidate-', flag1,'-',flag2,'-',num2str(nq), '-', num2str(nps),'.data'];
    visualName = [datasetName, '-pt-with-neighbor-consolidate-', flag1,'-',flag2,'-',num2str(nq), '-', num2str(nps), '-constraints.data'];
elseif isequal (exploreMethod, 'plus') 
    fileName = [datasetName, '-pt-with-neighbor-consolidate-plus-', flag1,'-',flag2,'-',num2str(nq), '-', num2str(nps),'.data'];
    visualName = [datasetName, '-pt-with-neighbor-consolidate-plus-', flag1,'-',flag2,'-',num2str(nq), '-', num2str(nps), '-constraints.data'];
end
