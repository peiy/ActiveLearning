function fathest_first_withTC (nq, nps, trainData, testData, wholeData, datasetName, num, clusterMethod, secondPhase)
n1 = length(trainData);
n2 = length(testData);
n = n1+n2;
...
for i = 0:nps:nq
    ...
    elseif i>0
        [neighborhood, j, selectPairs] = explorer (i, trainData, testData, wholeData);
        if j > 0
...
cd (datasetName);
if isequal (secondPhase, 'consolidate')
    fileName = [datasetName, '-farthest-first-withTC-',num2str(nq), '-', num2str(nps), '-', clusterMethod,'.data'];
    visualName = [datasetName, '-farthest-first-withTC-',num2str(nq), '-', num2str(nps), '-constraints.data'];
elseif isequal (secondPhase, 'minmax')
    fileName = [datasetName, '-minmax-',num2str(nq), '-', num2str(nps), '-', clusterMethod,'.data'];
    visualName = [datasetName, '-minmax-',num2str(nq), '-', num2str(nps), '-constraints.data'];
end    
